...
if velocity[vector2_x] > 0 {
	var tile_right = scr_tile_collide_at_points(tile_map_id, [bbox_right - 1, bbox_top], [bbox_right - 1, bbox_bottom - 1]); // [] - in-line arrays
	
	if tile_right {
		x = bbox_right & ~(tile_size - 1);
		x -= bbox_right - x;
		velocity[@ vector2_x] = 0; // @ - passes by reference
	}
} else {
